  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      ...
      <div className="bg-white rounded-xl border border-gray-100 shadow-sm p-4 mb-6 flex flex-wrap gap-3 items-center">
        ...
        <select value={city} onChange={e => setCity(e.target.value)} className="text-sm border border-gray-200 rounded-lg px-3 py-2 focus:outline-none focus:ring-2 focus:ring-coral-400">
        ...
        <div className="flex rounded-lg overflow-hidden border border-gray-200">
          {TYPES.map(t => (
            <button
              ...
              className={`px-3 py-2 text-sm font-medium transition-colors ${type === t.value ? 'bg-gray-900 text-white' : 'bg-white text-gray-600 hover:bg-gray-50'}`}
            >
              {t.label}
            </button>
          ))}
        ...
        <select value={sort} onChange={e => setSort(e.target.value)} className="text-sm border border-gray-200 rounded-lg px-3 py-2 focus:outline-none focus:ring-2 focus:ring-coral-400 ml-auto">
        ...
        <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-4">
          {universities.map(uni => (
            <Link
              ...
              className="bg-white rounded-xl border border-gray-100 shadow-sm p-5 hover:shadow-md hover:border-gray-200 transition-all duration-200 group"
            >
              <div className="flex items-start gap-3">
                ...
                >
                  {uni.name.split(' ').filter((w: string) => w.length > 2).slice(0, 2).map((w: string) => w[0]).join('') || uni.name.slice(0, 2)}
                </div>
                <div className="flex-1 min-w-0">
                  <h3 className="font-semibold text-gray-900 text-sm leading-tight group-hover:text-gray-700 transition-colors">
                    {uni.name}
                  </h3>
                  <div className="flex items-center gap-1 mt-1">
                    <MapPin className="w-3 h-3 text-gray-400" />
                    <span className="text-xs text-gray-500">{uni.city}</span>
                    <span className={`ml-1 text-xs px-1.5 py-0.5 rounded font-medium ${uni.type === 'devlet' ? 'bg-green-100 text-green-700' : 'bg-amber-100 text-amber-700'}`}>
                      {uni.type}
                    </span>
                  </div>
                </div>
